...
def merge_index(scan_path: Path, db_path: Path, reset: bool = False, limit_papers: int | None = None):
    ...
    if reset:
        ...
        ensure_schema(con)
    # Ensure schema DDL is durably written before large ingest begins.
    try:
        con.commit()
    except Exception:
        pass
    ...
    # Final commit to ensure all changes are durable before signaling done
    try:
        con.commit()
    except Exception:
        pass
    ...
    # Ensure WAL (if used) is checkpointed into the main db before swapping
    try:
