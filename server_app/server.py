...
class SearchRequestHandler(SimpleHTTPRequestHandler):
    ...
    def api_paths_get(self):
        try:
            return self._write_json({
                'ok': True,
                'data_dir': str(config.DATA_DIR),
                'papers_src': str(config.PAPERS_SRC),
    ...
    def api_tools_download(self):
        ...
        try:
            ...
            max_n = 50000
        try:
            out_path = Path(out_dir).expanduser().resolve() if out_dir else (config.DATA_DIR / 'papers')
        except Exception:
            out_path = config.DATA_DIR / 'papers'
            ...
            try:
                ...
                while total < max_n:
                    try:
                        ...
                        errors += 1
                        last_err = f'oai: {e}'
                        write_status({'ok': False, 'state': 'error', 'error': last_err, 'downloaded': total, 'skipped': skipped, 'errors': errors, 'pages': pages, 'last_http': last_http})
                        break
                    pages += 1
                    if not ids:
                        break
                    for bid in ids:
                        ...
                        try:
                            ...
                            if res == 'ok':
                                ...
                                errors += 1
                                try:
                                    last_http = (res.split(':', 1)[1] or '').strip()
                                except Exception:
                                    last_http = None
                                last_err = res
                            else:
                        ...
                        if (total + skipped + errors) % 10 == 0:
                            write_status({'ok': True, 'state': 'downloading', 'downloaded': total, 'skipped': skipped, 'errors': errors, 'pages': pages, 'last_id': bid, 'last_http': last_http, 'last_error': last_err})
                        time.sleep(sleep_s)
                    if not token:
                        break
                    time.sleep(sleep_s)
                write_status({'ok': True, 'state': 'done', 'downloaded': total, 'skipped': skipped, 'errors': errors, 'pages': pages, 'last_http': last_http, 'last_error': last_err})
            except Exception as e:
    ...
    def api_index_build(self):
        ...
        def _verify_db(path: Path) -> tuple[bool, str | None]:
            try:
                import sqlite3 as _sqlite3
                con = _sqlite3.connect(str(path))
                try:
                    # ensure core tables exist
                    for t in ('token','paper','chunk','posting'):
                        con.execute(f'SELECT COUNT(1) FROM {t}')
                    return True, None
                finally:
                    try: con.close()
                    except Exception: pass
        ...
        def worker():
            ...
            try:
                ...
                # For small datasets, avoid WAL entirely to ensure a single-file DB
                small_dataset = False
                try:
                    total = compute_total_papers()
                    small_dataset = bool(total and total <= 10000)
                except Exception:
                    small_dataset = True
                if build_mode == 'sharded':
                    ...
                    if small_dataset:
                        os.environ['PROJECTSEARCHBAR_FAST_BUILD'] = '1'
                        os.environ.pop('PROJECTSEARCHBAR_LOW_MEM', None)
                    else:
                        if low_mem:
                    ...
                    if commit_papers is not None and commit_papers > 0:
                        os.environ['PROJECTSEARCHBAR_COMMIT_PAPERS'] = str(commit_papers)
                    idx_mod.main(idx_args)  # type: ignore[arg-type]
                    # Verify then swap into place
                    ok, err = _verify_db(build_db)
                    if not ok:
                        write_status({'ok': False, 'state': 'error', 'error': f'build_db_invalid: {err}'})
                        return
                    ...
                    # Clean up any WAL/SHM from the build db and destination
                    try:
                        for suffix in ('-wal', '-shm'):
                            p1 = Path(str(build_db) + suffix)
                            p2 = Path(str(config.DB_PATH) + suffix)
                            try: p1.unlink(missing_ok=True)  # type: ignore[arg-type]
                            except Exception: pass
                            try: p2.unlink(missing_ok=True)  # type: ignore[arg-type]
                            except Exception: pass
                    except Exception:
                        pass
                    write_status({'ok': True, 'state': 'done'})
                    ...
                    if small_dataset:
                        # Build directly into final DB to avoid WAL/swap complexity
                        ...
                        if limit:
                            idx_args += ['--limit-papers', str(limit)]
                        write_status({'ok': True, 'state': 'indexing'})
                        print('[index_build] Direct merge (small dataset) to', target_db)
                        os.environ['PROJECTSEARCHBAR_STATUS'] = str(status_path)
                        if SearchRequestHandler._build_id:
                            os.environ['PROJECTSEARCHBAR_BUILD_ID'] = SearchRequestHandler._build_id
                        os.environ['PROJECTSEARCHBAR_FAST_BUILD'] = '1'
                        ...
                        if commit_papers is not None and commit_papers > 0:
                            os.environ['PROJECTSEARCHBAR_COMMIT_PAPERS'] = str(commit_papers)
                        idx_mod.main(idx_args)  # type: ignore[arg-type]
                        ok, err = _verify_db(target_db)
                        if not ok:
                            write_status({'ok': False, 'state': 'error', 'error': f'db_invalid: {err}'})
                            return
                        # Ensure no stray WAL/SHM remain
                        try:
                            for suffix in ('-wal', '-shm'):
                                p = Path(str(target_db) + suffix)
                                try: p.unlink(missing_ok=True)  # type: ignore[arg-type]
                                except Exception: pass
                        except Exception:
                            pass
                        write_status({'ok': True, 'state': 'done'})
                        ...
                        if commit_papers is not None and commit_papers > 0:
                            os.environ['PROJECTSEARCHBAR_COMMIT_PAPERS'] = str(commit_papers)
                        idx_mod.main(idx_args)  # type: ignore[arg-type]
                        # Verify then atomically swap in the new DB
                        ok, err = _verify_db(build_db)
                        if not ok:
                            write_status({'ok': False, 'state': 'error', 'error': f'build_db_invalid: {err}'})
                            return
