      ...
      const t = setInterval(async () => {
        ...
        try {
          ...
          if (s.state === 'downloading') {
            ...
            const extra = s.last_id ? `, last ${s.last_id}${s.last_http?` [${s.last_http}]`:''}` : (s.last_http?` [${s.last_http}]`:'');
            setAnimatedStatus(dlStatus, `Downloading — pages ${pg}, downloaded ${d}/${mx}${sk?`, skipped ${sk}`:''}${er?`, errors ${er}`:''}${extra}`, true);
          } else if (s.state === 'done') {
            const d = s.downloaded||0, sk=s.skipped||0, er=s.errors||0;
            setAnimatedStatus(dlStatus, `Download complete — ${d} ok${sk?`, ${sk} skipped`:''}${er?`, ${er} errors`:''}${s.last_http?` (last HTTP ${s.last_http})`:''}`, false);
            clearInterval(t);
            btnToolsDownload.disabled = false; btnToolsDownload.textContent = orig || 'Download Papers';
          } else if (s.state === 'error') {
            setAnimatedStatus(dlStatus, `Download error: ${s.error||''}${s.last_http?` (HTTP ${s.last_http})`:''}`, false);
            clearInterval(t);
            btnToolsDownload.disabled = false; btnToolsDownload.textContent = orig || 'Download Papers';
